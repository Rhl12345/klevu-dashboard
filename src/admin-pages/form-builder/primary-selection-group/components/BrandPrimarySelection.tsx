...
const BrandPrimarySelection = (props: IBrandPrimarySelectionProps) => {
  const { addedBrandData, setAddedBrandData } = props;

  const [brandData, setBrandData] = useState<IPrimarySelectGroupProduct[]>([]);
  ...
  const fetchPrimarySelectGroup = useCallback(
    async (pageIndex = 0): Promise<void> => {
      try {
        ...
        setBrandData(responseData.items);
  ...
  const handleAddSelectedBrand = async () => {
    try {
      ...
      setAddedBrandData([...addedBrandData, ...selectedRows]);
      toast.success("Products added successfully");

      setSelectedRows([]);
    } catch (error) {
      toast.error("Failed to add selected brands");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <div>
      ...
      >
        <Button
          type="button"
          variant="primary"
          onClick={() => handleAddSelectedBrand()}
...
export default BrandPrimarySelection;
