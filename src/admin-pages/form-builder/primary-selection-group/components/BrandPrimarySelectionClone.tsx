...
const BrandPrimarySelectionClone = (props: IBrandPrimarySelectionProps) => {
  const { addedBrandData, storeName, id } = props;

  const [brandCloneData, setBrandCloneData] = useState<
    IPrimarySelectGroupProduct[]
  >([]);
  ...
  const [selectedBrandProduct, setSelectedBrandProduct] =
  ...
  const handleModalOpen = useCallback(
    (type: IModalType, selectedBrandProduct: IPrimarySelectGroupProduct) => {
      setSelectedBrandProduct(selectedBrandProduct);
  ...
  const fetchPrimarySelectGroup = useCallback(
    async (pageIndex = 0): Promise<void> => {
      try {
        ...
        setBrandCloneData(responseData.items);
  ...
  useEffect(() => {
    setSelectedRows((prev) => [...prev, ...addedBrandData]);
  }, [addedBrandData]);
      ...
      {isModalOpen.isOpen &&
        isModalOpen.type === MODAL_TYPES.ACTIVE_INACTIVE && (
          <StatusChangeModel
            ...
            currentRowData={{
              recStatus:
                selectedBrandProduct?.recStatus === RecStatusValuebyName.Active
                  ? "active"
                  : "inactive",
              quantityName: "product",
              recordName: "product",
            }}
            title={`${selectedBrandProduct?.recStatus === RecStatusValuebyName.Active ? "Inactive" : "Active"} Status`}
            message={`Are you sure you want to ${selectedBrandProduct?.recStatus === RecStatusValuebyName.Active ? "inactive" : "active"} this product?`}
          />
        )}
    </div>
  );
...
export default BrandPrimarySelectionClone;
