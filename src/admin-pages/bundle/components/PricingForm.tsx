...
import { BUNDLE_PRICING_FORM_FIELD } from "@/constants/bundle/fields.constant";
...
const BundlePricingFormTab = ({
  ...
  const validationSchema = useMemo(() => {
    return getValidationSchema([BUNDLE_PRICING_FORM_FIELD]);
  }, [storeType]);

  const pricingFormikBag = useFormik({
    initialValues: {
      ...getInitialValuesFromMultipleFields(BUNDLE_PRICING_FORM_FIELD),
      ...initialData,
    },
    validationSchema,
      ...
      <form onSubmit={pricingFormikBag.handleSubmit}>
        <FormikProvider value={pricingFormikBag}>
          <div className="w-full grid grid-cols-4 gap-4">
            {Object.entries(BUNDLE_PRICING_FORM_FIELD).map(([key, field]) => (
              <div
                key={key}
                className={`${
                  field.fullWidth
...
export default BundlePricingFormTab;
