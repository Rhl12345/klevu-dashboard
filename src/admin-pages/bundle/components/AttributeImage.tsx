...
import { getAttributesImageData } from "@/services/product/productAttributes.service";
 ...
 * ```
 */
const BundleAttributeImagesSection = ({
  bundleId,
  handleTabChange,
  isEditPage,
}: {
  ...
  const fetchAttributesImageData = async () => {
    try {
      const attributes = await getAttributesImageData(bundleId);
      setData(attributes);
    } catch (error) {
    ...
    return (
      <div className="w-full py-2 px-4">
        <div className="grid grid-cols-5 gap-4">
          {row.original.media.map(
            (mediaItem: IAttributeMedia, index: number) => (
              <div
                ...
                className={`relative  flex flex-col gap-2 ${!isEditPage ? "items-center" : ""}`}
              >
                <Image
                  src={mediaItem.url}
                  alt={`Media ${index + 1}`}
                ...
                <div className="grid grid-cols-3 gap-1">
                  {!isEditPage ? (
                    <Text size="sm" className="col-span-full !text-center">
                      {mediaItem.name}
                    </Text>
                  ) : (
                    <>
          ...
          {row.original.media.length < 5 && isEditPage && (
            <label
  ...
  return (
    <div className="w-full rounded-none content border border-gray-light dark:border-gray-dark bg-body-light dark:bg-body-dark">
      <div className="flex justify-between items-center px-4 lg:px-6 pt-4 xl:pt-6">
        <Text size="lg" className={!isEditPage ? "font-semibold" : ""}>
          Attributes Images
        </Text>
        {isEditPage ? (
...
export default BundleAttributeImagesSection;
